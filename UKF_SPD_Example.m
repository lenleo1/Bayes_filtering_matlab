...
% [3] John D'Errico (2020). nearestSPD, MATLAB Central File Exchange. Retrieved September 14, 2020.
% by lenleo
% Update: UKF needs chol() operation to covariance matrix P, which may not
% always be positive definite. Here we use nearstSPD() function to get the
% nearest (in Frobenius norm) Symmetric Positive Definite matrix to P.
% Phat = nearestSPD(P);
clear;clc
n=2;                                                                       % number of state
...
%Output:
%       X: Sigma points
Phat = nearestSPD(P);                                                      % get the nearest Symmetric Positive Definite matrix to P
A = c*chol(Phat)';
Y = x(:,ones(1,numel(x)));
xsigma = [x Y+A Y-A]; 
end
